...
export const MockupHeader = ({
  ...
  <div className={cn("mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between", className)}>
    <div className="min-w-0">
      ...
      <h1 className="text-3xl font-semibold tracking-tight text-slate-950 font-heading">{title}</h1>
...
export const MockupCard = ({ children, className }: { children: ReactNode; className?: string }) => (
  <motion.section
    ...
    className={cn("enterprise-card rounded-2xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md", className)}
  >
    {children}
  </motion.section>
);
...
export const MockupPanel = ({
  ...
  <MockupCard className={cn("overflow-hidden", className)}>
    {title || actions ? (
      <div className="flex min-h-14 items-center justify-between gap-3 border-b border-slate-100 bg-white px-5 py-4">
        {title ? <h2 className="text-lg font-medium text-slate-950 font-heading">{title}</h2> : <span />}
        {actions}
      </div>
    ) : null}
...
export const MockupStatCard = ({
  ...
  <MockupCard className={cn("min-h-[132px] p-5", className)}>
    <div className="flex items-start justify-between gap-4">
      <div className="min-w-0">
        <p className="text-sm font-medium text-slate-600">{title}</p>
        <div className="mt-3 break-words text-3xl font-bold leading-none text-slate-950 font-heading tabular-nums">{value}</div>
        {subtitle ? <p className="mt-3 text-xs leading-5 text-slate-500">{subtitle}</p> : null}
      </div>
      {Icon ? (
        <span className={cn("flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border", toneStyles[tone])}>
...
export const IconAction = ({
  ...
  <button
    type="button"
    onClick={onClick}
    className={cn("flex w-full items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-left transition-colors hover:border-slate-300 hover:bg-slate-50", className)}
  >
    <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-[var(--role-accent)]">
...
export const SelectShell = ({ children, className }: { children: ReactNode; className?: string }) => (
  <div className={cn("h-9 rounded-lg border border-slate-200 bg-white px-3 text-xs font-medium text-slate-600 shadow-sm", className)}>
