...
import { MockupHeader, MockupPage, MockupPanel } from "@/components/mockup/MockupUI";
    ...
    <MockupPage>
      <MockupHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-sm border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-semibold text-slate-600">
      ...
      <div className="flex flex-wrap gap-3 rounded-sm border border-slate-200 bg-white p-4 shadow-sm">
        {canScopeMarkets && (
          <div className="space-y-1.5">
            ...
            <select
              ...
              className="h-9 min-w-[200px] rounded-sm border-2 border-slate-300 bg-white px-3 text-sm focus:border-primary focus:outline-none"
            >
        ...
        <div className="space-y-1.5">
          ...
          <Input type="datetime-local" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="h-9 w-[190px] border-2 border-slate-300 rounded-sm focus-visible:border-primary focus-visible:ring-0" />
        </div>
        <div className="space-y-1.5">
          <p className="text-xs font-bold uppercase tracking-wider text-slate-500">To</p>
          <Input type="datetime-local" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="h-9 w-[190px] border-2 border-slate-300 rounded-sm focus-visible:border-primary focus-visible:ring-0" />
        </div>
        <div className="space-y-1.5 flex-1 min-w-[220px]">
          ...
          <div className="relative">
            ...
            <Input
              className="pl-9 border-2 border-slate-300 rounded-sm focus-visible:border-primary focus-visible:ring-0 h-9"
        ...
        {hasFilters && (
          <div className="flex items-end">
            <Button variant="outline" className="h-9 rounded-sm border-slate-300 font-bold" onClick={() => { setSearch(""); setDateFrom(""); setDateTo(""); setSelectedMarketId("all"); }}>
              Reset
            </Button>
          </div>
        )}
      ...
      {/* Table */}
      <MockupPanel title={`Activity Records — ${filteredEvents.length} of ${events.length}`}>
              ...
              <TableBody>
                {filteredEvents.map((event) => (
                  <TableRow key={event.id} className="hover:bg-slate-50">
                    ...
                    <TableCell>
                      <span className="rounded-sm border border-slate-200 bg-slate-100 px-2 py-0.5 text-[11px] font-semibold capitalize text-slate-600">
                        {event.actorRole}
                      </span>
                    </TableCell>
                    <TableCell>
                      <span className="rounded-sm border border-slate-200 bg-slate-100 px-2 py-0.5 font-mono text-[11px] text-slate-700">
                        {event.action}
                      </span>
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <Button size="sm" variant="outline" className="h-7 rounded-sm border-slate-300 px-2 text-xs font-bold" onClick={() => setSelectedEventId(event.id)}>
                        View
                      </Button>
                    </TableCell>
                  </TableRow>
          ...
          {selectedEvent && (
            <div className="mt-4 space-y-4">
              ...
              <div className="rounded-sm border border-slate-200 bg-slate-50 p-4">
                ...
                <pre className="max-h-[400px] overflow-auto whitespace-pre-wrap break-words rounded-sm border border-slate-200 bg-white p-3 font-mono text-xs text-slate-700">
              ...
              <div className="sticky bottom-0 border-t border-slate-200 bg-white pt-3 sm:hidden">
                <button type="button" onClick={() => setSelectedEventId(null)} className="flex h-11 w-full items-center justify-center rounded-sm border border-slate-200 bg-slate-50 text-sm font-bold text-slate-900">
                  Close
                </button>
              </div>
            </div>
