...
const StallsPage = () => {
  ...
  // ── Vendor view ───────────────────────────────────────
  if (role === "vendor") {
    const myStall = (myStallQuery.data?.stalls || []).find((s) => s.status === "active");
    ...
    return (
      <PageLayout>
        ...
        <Card>
          <CardHeader className="flex min-h-12 flex-row items-center justify-between gap-3 border-b border-slate-100 bg-white px-4 py-3">
            <CardTitle className="text-base font-medium">{t("stalls:availableStalls")}</CardTitle>
          </CardHeader>
          <CardContent className="p-4">
            {stallsQuery.isPending ? (
              <LoadingState rows={4} itemClassName="h-20 rounded-lg" />
              ...
              <>
                <div className="mb-4 flex flex-wrap gap-1 rounded-lg border border-slate-200 bg-slate-50 p-1">
                  <button key="available" type="button" onClick={() => setStatusFilter("available")}
                    className={`h-8 rounded-lg px-3 text-xs font-bold transition-colors ${statusFilter === "available" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-900"}`}>
                    {t("stalls:filterAvailable")}
                  </button>
                </div>
                        ...
                        <div className="grid gap-2 grid-cols-3 sm:grid-cols-4 xl:grid-cols-6">
                          {rowStalls.map((stall) => (
                            <button key={stall.id} type="button"
                              ...
                              className={`min-h-[72px] rounded-lg border p-2.5 text-center transition-colors ${statusClasses[stall.status]} cursor-pointer`}>
        ...
        <Dialog open={Boolean(selectedStall)} onOpenChange={(open) => !open && setSelectedStall(null)}>
          <DialogContent className="rounded-lg sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>{t("stalls:applyForStallDesc")}</DialogDescription>
            </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Input type="date" value={reserveDates.startDate} onChange={(e) => setReserveDates((c) => ({ ...c, startDate: e.target.value }))} className="border-slate-300 rounded-lg" />
                  </div>
                  <div className="space-y-1.5">
                    <Label className="font-bold text-slate-700">End Date</Label>
                    <Input type="date" value={reserveDates.endDate} onChange={(e) => setReserveDates((c) => ({ ...c, endDate: e.target.value }))} className="border-slate-300 rounded-lg" />
                  </div>
                </div>
                <Button className="w-full rounded-lg shadow-none bg-primary hover:bg-primary/90 font-bold"
                  disabled={reserveStall.isPending}
                  onClick={() => reserveStall.mutate({ stallId: selectedStall.id, startDate: reserveDates.startDate, endDate: reserveDates.endDate })}>
                  {reserveStall.isPending ? t("common:submitting") : t("stalls:applyForStall")}
                </Button>
              </div>
            )}
          </DialogContent>
