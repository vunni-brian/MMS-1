...
const severityConfig = {
  ...
  warning: { label: "Warning", className: "bg-yellow-100 text-yellow-700 border-yellow-200", icon: AlertTriangle },
  info: { label: "Info", className: "bg-blue-100 text-blue-700 border-blue-200", icon: Clock },
};

const statusConfig = {
  open: { label: "Open", className: "bg-orange-100 text-orange-700" },
  watching: { label: "Watching", className: "bg-purple-100 text-purple-700" },
 ...
 const openCount = alerts.filter((alert) => alert.status === "open").length;
  ...
  const errorEntries = useMemo(() => {
    ...
    if (paymentsQuery.isError) entries.push({ label: "Payments", error: paymentsQuery.error as Error, retry: () => paymentsQuery.refetch() });
    if (ticketsQuery.isError) entries.push({ label: "Complaints", error: ticketsQuery.error as Error, retry: () => ticketsQuery.refetch() });
    if (utilityChargesQuery.isError) entries.push({ label: "Utility Charges", error: utilityChargesQuery.error as Error, retry: () => utilityChargesQuery.refetch() });
    if (penaltiesQuery.isError) entries.push({ label: "Penalties", error: penaltiesQuery.error as Error, retry: () => penaltiesQuery.refetch() });
      ...
      <Card className="max-w-2xl border-red-200 bg-red-50">
          <CardContent className="p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-semibold text-red-900">Some data sources failed to load</h3>
                <p className="text-sm text-red-700">Alerts may be incomplete until all data sources recover.</p>
              </div>
            </div>
              ...
              {errorEntries.map((entry) => (
                <div key={entry.label} className="flex items-center justify-between rounded-md border border-red-200 bg-white px-4 py-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-xs text-red-600">{entry.error?.message ?? "Unknown error"}</p>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Review exceptions, payment failures, and operational warnings across the platform.
          </p>
        </div>
        <Button 
          variant="outline" 
      ...
      {/* Alerts Table */}
            ...
            <DataTableFrame>
              {isLoading ? (
                ...
                <EmptyState title="No alerts found" description="There are no current alerts matching these filters." icon={ShieldAlert} />
              ) : (
      ...
      <Card className="border-slate-200 bg-white">
        <CardHeader className="pb-3">
          <CardTitle className="text-base text-slate-900">Notification Rules</CardTitle>
          <CardDescription className="text-xs text-slate-500">
            Current routing configuration. Manage via System Settings.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {[
              ["Critical alerts", true, "bg-red-50 border-red-200"],
              ["Payment failures", true, "bg-yellow-50 border-yellow-200"],
              ["Daily summary", true, "bg-blue-50 border-blue-200"],
              ["Low priority updates", false, "bg-slate-50 border-slate-200"],
