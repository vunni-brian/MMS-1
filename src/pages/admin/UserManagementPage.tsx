  ...
  if (isLoading) {
    return (
      <ConsolePage>
        ...
        <LoadingState rows={2} itemClassName="h-[320px] rounded-xl" />
      </ConsolePage>
    );
  }
                      ...
                      <div key={group.title}>
                        ...
                        <div className="grid gap-1.5">
                          {available.map((permission) => (
                            <label
                              key={permission}
                              className="flex items-start gap-2 rounded-md border border-border/70 bg-background px-2.5 py-2 text-sm"
                            >
                              <Checkbox
  ...
  <div className="grid gap-3 xl:grid-cols-[0.9fr_1.1fr]">
    ...
    >
      {[
        ...
        <div key={item.label} className="flex items-center gap-3 rounded-md border border-border/70 bg-background p-2.5">
  ...
  return (
    <div className="grid gap-2">
      {rows.slice(0, 12).map((account) => (
        <div key={account.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 bg-background p-2.5">
